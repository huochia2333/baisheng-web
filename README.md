...
| 角色 | 默认入口 | 当前模块 |
| --- | --- | --- |
| `administrator` | `/admin/home` | 全局首页、公告管理、反馈管理、系统设置；旅游业务下的订单、推荐树、团队、人员、操作记录、佣金、任务和审核；批发业务下的批发订单、订单认领、物流、人员管理、推荐树、佣金和提成 |
...
| `client` | `/client/home` | 全局首页；旅游业务下的订单、推荐树；关联批发客户身份后可见批发订单、物流、推荐树和佣金 |
...
- `/admin/announcements` 和 `/admin/feedback` 是管理员全局管理页面，不归属于旅游业务或批发业务。
...
- 当前阶段管理员固定可见旅游业务和批发业务；业务员固定仅可见批发业务；地推固定仅可见旅游业务。
- 人员管理不再提供手动勾选“可见旅游业务 / 可见批发业务”的入口。客户等非固定角色的批发可见范围由批发客户关联等业务流程开通。
- 业务员的邀请码只生成批发注册链接，地推的邀请码只生成旅游注册链接；注册链接会把新客户写入对应业务的推荐关系。
- 已知但未授权的同工作台模块展示访问错误页，不再显示占位入口。
- 服务端权限判断必须使用 Supabase 验证后的用户身份读取角色和状态；客户端会话只用于界面状态和会话同步，不能作为服务端放行依据。
...
- `admin-people/`：管理员人员管理，列表、详情弹窗、备注、VIP 单元格和视图状态按同层模块拆分，配合 `lib/admin-people*.ts`。
...
- 业务入口不再由管理员在人员管理里手动开关；管理员固定同时可见旅游业务和批发业务，业务员固定只可见批发业务，地推固定只可见旅游业务，其它普通角色默认按旅游业务进入，批发客户绑定后由批发客户关联流程开通批发可见范围。
- 批发人员管理包含“客户”和“业务员账户”两个页签，不再拆成两个侧栏入口；批发客户以客户唯一标识名称作为业务上的唯一客户名，可以先不绑定系统账号，由业务员登记联系方式、其他名称和客户来源；客户后续完成注册后，管理员或负责该客户的业务员可以在客户详情里选择客户注册账号进行关联合并，系统不会新建第二个批发客户记录，而是把注册账号绑定到原客户并开通批发业务可见权限，客户登录后可以查看同一客户名下的订单、物流、推荐关系、佣金和 VIP 权益。
...
- 管理员人员管理只展示已完成邮箱验证的账号；账号列表只直接展示账号、身份/状态、客户标记、VIP 和操作，城市、备注、推荐关系、团队、邀请码、业务可见范围、创建时间等完整资料通过点击账号后的详情弹窗查看；账号身份、状态、城市和客户标记可在详情弹窗下方调整，可见业务只按角色和业务流程自动同步，不提供手动勾选入口；推荐树、团队和佣金分别维护自己的业务边界。
- 人员管理里的“我的备注”按当前登录账号单独保存，只显示给填写人本人；管理员、业务员和地推只能给自己人员页内可见的账号填写或清空备注。
...
- 工作台业务板块页头默认不展示统计小卡片；人员管理保留人数和状态概览，列表只保留关键列，更多账号资料放入详情弹窗，其他板块以筛选、列表、表格和分段切换承载主要信息。
...
- 账号可见业务由 Supabase 里的工作区业务可见范围记录、角色固定规则和对应 RPC 提供，Web 只负责读取后过滤导航和路由；业务员固定批发、地推固定旅游、管理员固定双业务。
