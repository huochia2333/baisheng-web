...
- `/admin/settings` 是管理员全局系统设置页面，不归属于旅游业务或批发业务；页面内按业务模块注册的设置区块和全局汇率设置分区。
...
- `lib/workspace-business-modules.ts`：旅游业务和批发业务的模块清单，集中声明业务导航、角色页面能力、页面入口类型和系统设置区块。
...
- 旅游业务和批发业务的导航、角色页面能力、页面入口和设置区块由 `lib/workspace-business-modules.ts` 注册；新增、删除或裁剪业务时优先改业务模块清单，不在 `workspace-config.ts` 或页面入口里散落判断。
...
- `admin-system-settings/`：管理员全局系统设置，业务设置页签由业务模块清单注册；旅游业务注册服务费、服务价格和旅游佣金区块，批发业务注册批发佣金区块，汇率设置保持全局区块，不挂在任何业务侧栏下。
...
- 当前业务模块清单注册 `tourism` 和 `wholesale` 两个业务。导航分组、角色可用模块、旅游/批发页面入口和系统设置业务页签都从 `lib/workspace-business-modules.ts` 组装；工作台总配置不再直接维护每个业务的导航细节。
...
- 系统设置是管理员全局父级入口，内部按业务模块注册的设置区块和全局汇率区块展示；旅游业务设置维护旅游订单服务费、服务价格、服务折扣和旅游相关佣金，批发业务设置维护采购订单业务员佣金和采购订单推荐佣金，汇率设置维护自动获取币种、手动补充汇率和历史记录。
...
- 账号可见业务由 Supabase 里的工作区业务可见范围记录、角色固定规则和对应 RPC 提供，Web 只负责读取后结合业务模块清单过滤导航和路由；业务员固定批发、地推固定旅游、管理员固定双业务。当前登录用户的业务权限读取异常或返回空时，Web 会按已验证角色和 active 状态使用固定默认业务范围，避免管理员工作台因临时读取失败误显示为没有业务入口。
