...
- `dashboard-home/`：各角色首页问候、公告和个人待办；待办的查询、保存状态、弹窗和展示拆在同层模块中，不放进首页 Client/Page。
...
- 首页个人待办只属于当前登录账号，支持新增、编辑、完成/恢复、删除、重要标记、截止日期、备注和全部/未完成/已完成筛选；快速添加栏的输入框、截止日期、重要标记和提交按钮保持同一行同一高度；数据读取和写入通过 `lib/user-todos.ts` 与 Supabase `user_todo_items` / `create_user_todo` / `update_user_todo` / `delete_user_todo` 对接。
