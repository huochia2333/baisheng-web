...
- `LOGISTICS_SOURCE_STATUS_API_URL` 和 `LOGISTICS_SOURCE_STATUS_API_TOKEN` 只配置为 Supabase Edge Function secrets，用于批发物流状态每日核对，不暴露到浏览器端；`LOGISTICS_SOURCE_SUPABASE_URL` 和 `LOGISTICS_SOURCE_SUPABASE_READONLY_KEY` 仅作为旧的直连备用配置。
...
- 批发客户、批发订单、物流、客户推荐和 1688 采购单认领的写入权限以数据库为准：管理员可以管理全部批发数据，财务和业务员只能管理自己创建或分配给自己的批发客户及其相关订单、物流费用和推荐关系，不能替其他业务员建单、转派客户或认领到其他人的客户名下。物流状态核对记录对管理员、财务和业务员全部可见，客户只查看自己绑定批发客户名下的物流状态。
...
- 批发客户推荐佣金按月统计，同一被推荐客户当月佣金由两套规则相加：批发订单人民币金额默认按 `2%` 计算，运单数量超过 `200` 单加 `50 USD`、超过 `1000` 单加 `200 USD`、超过 `5000` 单加 `500 USD`，美元奖励按当前 USD 兑人民币汇率折算后计入合计。运单数量优先使用批发物流状态核对记录，同时兼容历史物流费用记录；同一个物流号在两处都存在时只计一次。上述比例、门槛和奖励金额在批发业务设置中维护。
- 一笔批发订单可以关联多条 1688 采购订单，也可以关联多条物流状态或物流费用记录；批发订单列表直接展示这两类关联记录的摘要，搜索订单时同时匹配关联采购单、物流单号和物流状态，并支持按下单日期选择开始和结束日期查看特定时间段的订单；选择好当前范围后，可以生成业务口径的 AI 订单评估，评估内容使用普通中文展示，覆盖订单数量、结汇状态、客户支付金额、采购成本、各类费用、毛利表现、风险点和下一步建议。
...
- 物流模块以“物流号、客户名、当前状态”为核心。管理员、财务和业务员可以录入需要核对的物流号，系统每天核对一次第二物流库的状态；出现已送达、已签收、退回、丢失、取消、无法投递或异常终止后停止继续核对。历史物流费用记录继续展示来源工作流订单编号、国际订单号、目的地订单号、货代、进度和费用，供费用核对使用。
...
- 批发物流状态每日核对由 Supabase `wholesale-logistics-status-sync` Edge Function 执行；该函数优先调用物流库的 `logistics-status-lookup` 接口，主页面只读取本项目 Supabase 里的物流状态镜像表。
