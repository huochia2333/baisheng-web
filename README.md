...
- 物流模块以“物流号、客户名、当前状态”为核心。系统每天从第二物流库自动导入物流号、客户名和最新状态，并把客户名精确匹配到现有批发客户；未匹配到客户的记录仍对管理员、财务和业务员可见，方便后台处理。出现已送达、已签收、退回、丢失、取消、无法投递或异常终止后停止继续核对。历史物流费用记录继续展示来源工作流订单编号、国际订单号、目的地订单号、货代、进度和费用，供费用核对使用。
